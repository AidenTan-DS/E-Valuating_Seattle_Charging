"""
app_v3.py – Seattle EV Station Explorer
Run from project root: streamlit run interactive_map/app_v3.py

Tabs: Location (traffic + stations), Evaluation (scores), Prediction (ML grid map)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Prediction map builder (split into small functions for readability and testing)
# ─────────────────────────────────────────────────────────────────────────────
...
def main():  # pylint: disable=too-many-locals,too-many-statements  # Streamlit page layout requires many local variables and render calls in one function
    ...
    with tab2:
        ...
        adt_range = scored["mean_ADT"].max() - scored["mean_ADT"].min()
        adt_n = (scored["mean_ADT"] - scored["mean_ADT"].min()) / (adt_range + 1e-9)
        dens_range = scored["pop_density"].max() - scored["pop_density"].min()
        dens_n = (scored["pop_density"] - scored["pop_density"].min()) / (dens_range + 1e-9)
        dem_range = scored["demand_gap"].max() - scored["demand_gap"].min()
        dem_n = (scored["demand_gap"] - scored["demand_gap"].min()) / (dem_range + 1e-9)
        adt_n, dens_n, dem_n = adt_n.fillna(0), dens_n.fillna(0), dem_n.fillna(0)
